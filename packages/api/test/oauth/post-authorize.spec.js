...
describe('/oauth/authorize', () => {
  ...
  describe('POST', () => {
    it('should return login page if invalid login', async () => {
      // When
    ...
    it('should return permissions page if valid login', async () => {
      // When
      const response = await server.inject({
        method: 'POST',
    ...
    it('should redirect if authorized', async () => {
      // When
      const response = await server.inject({
        method: 'POST',
        url: `/oauth/authorize?app_id=${fixApp.appId}&redirect_uri=${fixApp.redirectUris[0]}`,
    ...
    it('should return 403 if invalid redirect URI', async () => {
      // When
      const response = await server.inject({
        method: 'POST',
    ...
    it('should return 403 if requesting non allowed scope', async () => {
      // When
      const response = await server.inject({
        method: 'POST',
        url: `/oauth/authorize?app_id=${fixApp.appId}&redirect_uri=${fixApp.redirectUris[0]}`,
